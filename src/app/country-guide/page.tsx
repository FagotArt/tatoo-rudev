"use client";
import GuideMap from "@/components/guidemap";
import Divider from "@/components/ui/divider";
import useMediaQuery from "@/lib/utils/useMediaQuery";
import React from "react";

const Section = ({ title, children }: any) => {
  return (
    <div className="max-w-[300px] md:max-w-[500px]">
      <div className="p-[1rem] w-full border-b-[1px] border-white font-bold mb-[1rem] text-[1.2rem]">{title}</div>
      <div className="font-['Helvetica']">{children}</div>
    </div>
  );
};

const Page = () => {
  const mobile = useMediaQuery("(max-width: 768px)");
  return (
    <div className="py-[120px]">
      <div className="text-4xl mb-[2rem] text-center font-bold">Country Guide</div>
      <Divider className="max-w-[200px] !mb-[2rem]" />
      <GuideMap width={mobile ? 300 : 700} containerClassName="mx-auto mb-[3rem]" />
      <div className="max-w-[calc(1000px+2rem)] mx-auto flex justify-center items-start flex-wrap flex-col md:flex-row gap-[1rem] md:gap-[2rem]">
        {/* <Section title="Tattoo Etiquette">Dos and don&apos;ts related to tattoos in various countries can help you to navigate your tattoo experiences respectfully.</Section>
        <Section title="Legal Considerations">
          Tattoo laws and regulations can vary from country to country.
          <br />
          Information on legal aspects, such as age restrictions and required consent, will help you comply with local regulations.
        </Section>
        <Section title="Pre-Travel Preparation">
          If you plan to get a tattoowhile traveling, this guide can help you be prepared, informed, and aware of what to expect in your destination country.
        </Section>
        <Section title="Respectful Engagement">
          Understanding cultural norms and customs can help clients engage with local artists and communities in a respectful and positive manner.
        </Section>
        <Section title="Cultural Exchange">
          Tattoo laws and regulations can vary from country to country. Information on legal aspects such as age restrictions and required consent, will help you comply with local
          regulations.
        </Section> */}
        <Section title="Before Your Tattoo">
          &#x2022; Get a good night’s sleep and stay hydrated
          <br />
          <br /> &#x2022; Avoid thinning your blood by avoiding alcohol, and caffeine
          <br />
          <br /> &#x2022; Wear comfortable clothes that allow easy access to the tattoo area. You may also get ink on the clothes you wear so avoid wearing anything you wouldn’t
          want damaged (or light-coloured clothing)
          <br />
          <br /> &#x2022; Eat a balanced meal and bring snacks for longer sessions
          <br />
          <br /> &#x2022; Ensure that the area is free from sunburn and free from any flared-up skin conditions.
          <br />
          <br /> &#x2022; For scar cover-up you must consult the artist prior to ensure the scar has had long enough to heal to be tattooed over.
        </Section>
        <Section title="During a Tattoo">&#x2022; Keep hydrated and feel free to snack to keep your blood sugars up.</Section>
        <Section title="After a Tattoo">
          Your artist will tell you how to take care of our tattoo in the hours and days after your tattoo. Different artists may have slightly different advice. Listen to each
          artist to ensure the best healing process for that tattoo. Consider purchasing the products that the artist or studio sells especially if you are new to tattoos and
          haven’t already been advised by others what you could use.
        </Section>
        <Section title="DO NOT">
          &#x2022; Touch, pick, scratch or rub the area
          <br />
          <br /> &#x2022; Shave the area
          <br />
          <br /> &#x2022; Put medicated ointments on the area
          <br />
          <br /> &#x2022; Apply suncream
          <br />
          <br /> &#x2022; Expose the healing tattoo to direct sunlight (definitely don’t sunbathe)
          <br />
          <br /> &#x2022; Soak your tattoo in water (shower rather than bath if this is an option)
          <br />
          <br /> Avoid scented and unnatural moisturisers
          <br /> <br /> Avoid wearing tight fitting clothes that don’t breathe well
        </Section>
      </div>
    </div>
  );
};

export default Page;
